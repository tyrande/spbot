...
@implementation CvVideoPhotoCamera
     ...
     ^(CMSampleBufferRef imageSampleBuffer, NSError *error)
     {
         if (error == nil && imageSampleBuffer != NULL)
         {
             ...
             dispatch_async(dispatch_get_main_queue(), ^{
                 ...
                 if (self.delegate) {
                     [self.delegatePhoto photoCamera:(id)self capturedImage:newImage];
                 }
...
- (void)createStillImageOutput;
{
    ...
    NSDictionary *outputSettings = [NSDictionary dictionaryWithObjectsAndKeys:AVVideoCodecJPEG, AVVideoCodecKey, [NSNumber numberWithFloat:0.9], AVVideoQualityKey,nil];
